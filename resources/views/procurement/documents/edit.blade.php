        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-200">
                <div class="p-4 sm:p-6 text-gray-900" x-data="{ documentTypeId: '{{ old('document_type_id', $document->document_type_id) }}' }">
                    ...
                    @if ($errors->any())
                        <div class="mb-6 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg text-sm shadow-sm">
                    ...
                    <form method="POST" action="{{ $updateRoute }}" enctype="multipart/form-data" novalidate>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                            ...
                            <div class="md:col-span-3">
                                ...
                                <textarea id="references" name="references" rows="3" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm text-sm">{{ old('references', $document->references) }}</textarea>
                            </div>

                            <div class="md:col-span-3">
                                <x-input-label for="activity_description" value="ເນື້ອໃນກິດຈະກຳທີ່ຈະປະຕິບັດ" />
                                <textarea id="activity_description" name="activity_description" rows="3" class="block mt-1 w-full border-gray-300 rounded-md shadow-sm text-sm" required>{{ old('activity_description', $document->activity_description) }}</textarea>
                            </div>
                        </div>
                        ...
                        <div class="mt-8" x-show="documentTypeId == '1'" x-data="{ 
                            ...
                            <h3 class="text-base md:text-lg font-bold border-b border-gray-200 pb-2 text-gray-800">ລາຍການເບີກຈ່າຍ</h3>
                                    ...
                                    <div class="p-4 bg-gray-50 md:bg-transparent rounded-xl border border-gray-200 md:border-0 md:p-3 grid grid-cols-1 sm:grid-cols-12 gap-3 md:gap-4 items-center relative">
                                        ...
                                        <div class="sm:col-span-3 flex justify-between items-center sm:grid sm:grid-cols-3 sm:gap-2 pt-2 sm:pt-0 border-t sm:border-0 border-gray-200">
                                            ...
                                            <div class="sm:col-span-1 text-right">
                                                <button type="button" @click="items.splice(index, 1)" x-show="items.length > 1" class="text-red-500 hover:text-red-700 text-xs sm:text-sm font-semibold p-1 rounded hover:bg-red-50 transition">
                                                    &times; ລຶບ
                                                </button>
                                            </div>
                                        </div>
                            ...
                            <button type="button" @click="items.push({ item_description: '', quantity: 1, unit_price: '' })" class="mt-4 inline-flex items-center text-sm font-semibold text-blue-600 hover:text-blue-800 transition">
                                + ເພີ່ມລາຍການໃໝ່
                            </button>
                        </div>
                        ...
                        <div class="flex flex-col-reverse sm:flex-row items-center justify-end mt-8 gap-3 sm:space-x-4 border-t pt-6">
                            <a href="{{ $cancelRoute }}" class="w-full sm:w-auto text-center px-4 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-900 bg-gray-100 hover:bg-gray-200 rounded-lg transition">
                                ຍົກເລີກ
                            </a>
    
                            @if(in_array($document->status, ['DRAFT', 'REJECTED']))
                                <button type="submit" name="action" value="save_draft"
                                    class="w-full sm:w-auto justify-center inline-flex items-center px-6 py-2.5 bg-white border border-gray-300 rounded-lg font-bold text-sm text-gray-700 uppercase shadow-sm hover:bg-gray-50 transition">
                                    ບັນທຶກສະບັບຮ່າງ
                                </button>
                            @endif

                            <button type="submit" name="action" value="submit" 
                                class="w-full sm:w-auto justify-center inline-flex items-center px-8 py-2.5 bg-blue-600 border border-transparent rounded-lg font-bold text-sm text-white uppercase shadow-md hover:bg-blue-700 transition">
