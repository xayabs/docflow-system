<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            {{-- ປຸ່ມ Export (ຖ້າມີການໃຊ້ງານ Export ເໝືອນທີ່ເຄີຍແນະນຳໄປ) --}}
            <div class="hidden sm:flex space-x-2">
                <a href="{{ route('secretary.history.approved.export', 'xlsx') }}" class="inline-flex items-center px-3 py-1.5 bg-green-600 text-white text-xs font-bold rounded-md hover:bg-green-700 transition shadow-sm">
                    Export Excel
                </a>
            </div>
        </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg rounded-xl border border-gray-200">
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <x-document-filter-form 
                        :action="route('secretary.history.approved')" 
                        :departments="$departments"
                        :statuses="$statuses" 
                        title-span="md:col-span-2"
                    />

                    {{-- ປຸ່ມ Export ສຳລັບຈໍມືຖື (ສະແດງສະເພາະມືຖື) --}}
                    <div class="block sm:hidden mb-4 flex gap-2">
                        <a href="{{ route('secretary.history.approved.export', 'xlsx') }}" class="flex-1 text-center py-2 bg-green-50 text-green-700 border border-green-200 rounded-lg text-sm font-semibold shadow-sm active:bg-green-100">
                            Export Excel
                        </a>
                    </div>
                                ...
                                @forelse ($documents as $document)
                                    <tr class="hover:bg-gray-50 transition">
                                        ...
                                        <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('secretary.documents.show', $document->id) }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-900 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 transition">
                                                ເບິ່ງລາຍລະອຽດ
                                            </a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                                            ບໍ່ມີປະວັດເອກະສານທີ່ໄດ້ກວດສອບຜ່ານ
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                    ...
                    <div class="block md:hidden space-y-4 mt-4">
                        @forelse ($documents as $document)
                            <div class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm relative">
                                ...
                                {{-- ແຖບສີບອກສະຖານະ (ສີຂຽວອ່ອນສຳລັບປະຫວັດຜ່ານ) --}}
                                <div class="absolute top-0 left-0 w-1.5 h-full bg-green-400 rounded-l-xl opacity-60"></div>
                                ...
                                <div class="pt-3 mt-3 border-t border-gray-100 pl-2 flex justify-end">
                                    <a href="{{ route('secretary.documents.show', $document->id) }}" class="inline-flex items-center text-sm font-semibold text-indigo-600 hover:text-indigo-800">
                                        ເບິ່ງລາຍລະອຽດ
