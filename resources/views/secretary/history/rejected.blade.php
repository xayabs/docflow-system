    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('ປະວັດເອກະສານທີ່ໃຫ້ປັບປຸງຄືນ') }}
            </h2>
            
            {{-- ປຸ່ມ Export (ຖ້າມີການໃຊ້ງານ Export ເໝືອນໜ້າ Approved) --}}
            <div class="hidden sm:flex space-x-2">
                <a href="#" class="inline-flex items-center px-3 py-1.5 bg-gray-200 text-gray-600 text-xs font-bold rounded-md hover:bg-gray-300 transition shadow-sm cursor-not-allowed" title="ລໍຖ້າການພັດທະນາ">
                    Export Excel
                </a>
            </div>
        </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg rounded-xl border border-gray-200">
                <div class="p-4 sm:p-6 text-gray-900">
                    ...
                    <x-document-filter-form 
                        :action="route('secretary.history.rejected')" 
                        :departments="$departments"
                        :statuses="$statuses"
                        title-span="md:col-span-2"
                    />

                    {{-- ປຸ່ມ Export ສຳລັບຈໍມືຖື --}}
                    <div class="block sm:hidden mb-4 flex gap-2">
                        <a href="#" class="flex-1 text-center py-2 bg-gray-100 text-gray-500 border border-gray-200 rounded-lg text-sm font-semibold shadow-sm cursor-not-allowed">
                            Export Excel
                        </a>
                    </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 border-b border-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">ປະເພດ</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($documents as $document)
                                    {{-- Highlight ແຖວສີແດງອ່ອນໆ ສຳລັບເອກະສານທີ່ຖືກປະຕິເສດ --}}
                                    <tr class="bg-red-50/30 hover:bg-red-50/60 transition">
                                        ...
                                        <td class="px-6 py-4 text-center text-sm text-gray-600">{{ $document->documentType->name ?? 'N/A' }}</td>
                                        ...
                                        <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-medium">
                                            <a href="{{ route('secretary.documents.show', $document->id) }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-900 bg-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm transition hover:shadow">
                                                ເບິ່ງລາຍລະອຽດ
                                            </a>
                                        </td>
                                    </tr>
                    ...
                    <div class="block md:hidden space-y-4 mt-4">
                        @forelse ($documents as $document)
                            {{-- Card ສຳລັບເອກະສານປະຕິເສດ ຈະມີຂອບສີແດງ ແລະ ພື້ນຫຼັງແດງອ່ອນໆ --}}
                            <div class="bg-red-50/20 border border-red-200 rounded-xl p-4 shadow-sm relative overflow-hidden">
                                
                                {{-- ແຖບສີແດງບອກສະຖານະປະຕິເສດດ່ວນ --}}
                                <div class="absolute top-0 left-0 w-1.5 h-full bg-red-400 opacity-80"></div>

                                {{-- ແຖວເທິງສຸດ: ລະຫັດເອກະສານ & ປ້າຍສະຖານະ --}}
                                <div class="flex justify-between items-start pl-2 mb-2">
                                    <span class="font-mono text-xs font-bold text-gray-700 bg-white border border-red-100 px-2 py-1 rounded shadow-sm">
                                ...
                                {{-- ກ່ອງລາຍລະອຽດຍ່ອຍ --}}
                                <div class="pl-2">
                                    <div class="grid grid-cols-2 gap-2 text-xs text-gray-600 bg-white p-2.5 rounded-lg border border-red-50">
                                        <div>
                                            <span class="block text-gray-400 mb-0.5">ປະເພດ</span>
                                            <span class="font-medium">{{ $document->documentType->name ?? 'N/A' }}</span>
                                        </div>
                                        <div>
                                            <span class="block text-gray-400 mb-0.5">ວັນທີສົ່ງ</span>
                                ...
                                <div class="pt-3 mt-3 border-t border-red-100 pl-2 flex justify-end">
                                    <a href="{{ route('secretary.documents.show', $document->id) }}" class="inline-flex items-center text-sm font-semibold text-red-600 hover:text-red-800 bg-white px-4 py-2 rounded-lg border border-red-200 shadow-sm transition">
                                        ເບິ່ງລາຍລະອຽດ
